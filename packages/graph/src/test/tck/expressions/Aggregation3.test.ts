...
describe("Aggregation3 - Sum", () => {
  test.fails("[1] Sum only non-null values - implicit grouping not supported", () => {
    // Original TCK:
    // Given:
    ...
    const results = executeTckQuery(graph, "MATCH (n) RETURN n.name, sum(n.num)");
  ...
  test("[Custom 1] sum() over integers with UNWIND", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, "UNWIND [1, 2, 3, 4, 5] AS x RETURN sum(x)");
    expect(results).toHaveLength(1);
  ...
  test("[Custom 3] sum() over floats with UNWIND", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, "UNWIND [1.5, 2.5, 3.0] AS x RETURN sum(x)");
    expect(results).toHaveLength(1);
  ...
  test("[Custom 4] sum() with negative numbers", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, "UNWIND [10, -5, 3, -2] AS x RETURN sum(x)");
    expect(results).toHaveLength(1);
  ...
  test("[Custom 5] sum() returns 0 for empty result set", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n:NonExistent) RETURN sum(n.num)");
  ...
  test("[Custom 6] sum() with WHERE filter", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n:A) WHERE n.num > 15 RETURN sum(n.num)");

...
describe("Temporal4 - Store Temporal Values", () => {
  ...
  test.fails(
    "[1] Should store and retrieve date property - function call in CREATE property not supported",
    () => {
      // Original TCK:
      ...
      const result = executeTckQuery(
        graph,
        "CREATE (a:A {d: date('1984-10-11')}) RETURN a.d",
      );
  ...
  test.fails(
    "[2] Should store and retrieve localtime property - function call in CREATE property not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[3] Should store and retrieve time property - function call in CREATE property not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[4] Should store and retrieve localdatetime property - function call in CREATE property not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[5] Should store and retrieve datetime property - function call in CREATE property not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[6] Should store and retrieve duration property - function call in CREATE property not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[7] Should store array of dates - function call in CREATE property not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[8] Should store array of datetimes - function call in CREATE property not supported",
    () => {
      // Original TCK:
      ...
      expect(dts.map(String)).toEqual([
        "1984-10-11T12:31:14+01:00",
        "1985-11-12T13:32:15+02:00",
      ]);
    },
  );
  ...
  test.fails(
    "[10] Should handle null propagation through temporal - requires MATCH first",
    () => {
      // Original TCK:
      // MATCH (a)
      ...
      const result = executeTckQuery(
        graph,
        "MATCH (a) RETURN date(a.nonexistent)",
      );
      expect(result).toHaveLength(1);
      expect(result[0]).toBeNull();
    },

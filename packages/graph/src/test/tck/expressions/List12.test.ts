...
describe("List12 - List Comprehension", () => {
  ...
  test.fails(
    "[4] Returning a list comprehension - unlabeled nodes (by design)",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[5] Using a list comprehension in a WITH - unlabeled nodes in pattern (by design)",
    () => {
      // Original TCK:
      // CREATE (a:A), (a)-[:T]->(:B), (a)-[:T]->(:C)
      // MATCH p = (n:A)-->()
  ...
  test.fails(
    "[6] Using a list comprehension in a WHERE - unlabeled nodes (by design)",
    () => {
      // Original TCK:
  ...
  test("[7] Fail when using aggregation in list comprehension - semantic validation not implemented", () => {
    ...
    expect(() =>
      executeTckQuery(
        graph,
        "MATCH (n:A) RETURN [x IN [1, 2, 3, 4, 5] | count(*)]",
      ),
    ).toThrow();
  });
  ...
  test("[Custom 1a] List comprehension with projection", () => {
    ...
    const results = executeTckQuery(
      graph,
      "RETURN [x IN [1, 2, 3] | x * 2] AS result",
    );
    expect(results).toHaveLength(1);
  ...
  test("[Custom 1b] List comprehension with filter", () => {
    ...
    const results = executeTckQuery(
      graph,
      "RETURN [x IN [1, 2, 3, 4, 5] WHERE x > 2] AS result",
    );
    expect(results).toHaveLength(1);
  ...
  test("[Custom 1c] List comprehension with filter and projection", () => {
    ...
    const results = executeTckQuery(
      graph,
      "RETURN [x IN [1, 2, 3] WHERE x > 1 | x * 2] AS result",
    );
    expect(results).toHaveLength(1);
  ...
  test.fails(
    "[Custom 1] Collect nodes into a list - collect(n.name) property access in aggregate not supported",
    () => {
      ...
      expect(results).toHaveLength(1);
      expect((results[0] as string[]).sort()).toEqual([
        "Alice",
        "Bob",
        "Charlie",
      ]);
  ...
  test.fails(
    "[Custom 2] Collect with ordering - collect(n.num) property access in aggregate not supported",
    () => {
  ...
  test.fails(
    "[Custom 3] Filter before collect - collect(n.num) property access in aggregate not supported",
    () => {
      // Grammar limitation: collect(n.num) - property access in aggregate function not supported
      const graph = createTckGraph();
      executeTckQuery(
  ...
  test("[Custom 4] Count nodes after filtering", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) WHERE n.num > 2 RETURN count(n)",
    );

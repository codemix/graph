...
describe("Graph5 - Node and edge label expressions", () => {
  test.fails(
    "[1] Single-labels expression on nodes - label predicate in RETURN not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[2] Single-labels expression on relationships - label predicate not supported",
    () => {
  ...
  test.fails(
    "[3] Conjunctive labels expression on nodes - multi-label not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[4] Conjunctive labels expression with varying order - multi-label not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (a) WHERE a:A:C RETURN a.name",
      );
      expect(results).toHaveLength(1);
  ...
  test.fails(
    "[5] Label expression on null - OPTIONAL MATCH not fully supported",
    () => {
      // Original TCK:
  ...
  test("[Custom 2] Filter relationships by type in MATCH", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A)-[:T1]->(b:B) RETURN a.name, b.name",
    );

...
describe("Comparison4 - Combination of Comparisons", () => {
  test.fails(
    "[1] Handling long chains of operators - chained comparisons not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[custom-1] Combined < and = conditions with AND - cross-node property comparison not supported",
    () => {
      // Original test:
  ...
  test("[custom-2] Equality and inequality combined", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (:A {x: 1, y: 1}), (:A {x: 1, y: 2}), (:A {x: 2, y: 2})",
    );
  ...
  test("[custom-3] Less than and greater than combined", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (:A {a: 1, b: 5}), (:A {a: 3, b: 3}), (:A {a: 5, b: 1})",
    );

    // Find nodes where a < 4 AND b > 2
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) WHERE n.a < 4 AND n.b > 2 RETURN n.a, n.b",
    );
  ...
  test("[custom-5] Nested AND and OR with comparisons", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (:A {x: 1, y: 10}), (:A {x: 5, y: 5}), (:A {x: 10, y: 1})",
    );
  ...
  test.fails(
    "[custom-7] Cross-node property comparison - cross-variable property comparison not supported",
    () => {
      // Original test:
  ...
  test.fails(
    "[custom-8] Cross-node property equality - cross-variable property comparison not supported",
    () => {
      // Original test:
      // MATCH (a:A)-[:R]->(b:B) WHERE a.num = b.num RETURN a.num, b.num
      const graph = createTckGraph();
  ...
  test("[custom-9] NOT with comparison operators", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) WHERE NOT (n.num = 2) RETURN n.num",
    );

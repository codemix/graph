...
describe("Aggregation2 - Min and Max", () => {
  test("[1] `max()` over integers", () => {
    ...
    const results = executeTckQuery(graph, `UNWIND [1, 2, 0, null, -1] AS x RETURN max(x)`);
  ...
  test("[2] `min()` over integers", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, `UNWIND [1, 2, 0, null, -1] AS x RETURN min(x)`);
    expect(results).toHaveLength(1);
  ...
  test("[3] `max()` over floats", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, `UNWIND [1.0, 2.0, 0.5, null] AS x RETURN max(x)`);
    expect(results).toHaveLength(1);
  ...
  test("[4] `min()` over floats", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, `UNWIND [1.0, 2.0, 0.5, null] AS x RETURN min(x)`);
    expect(results).toHaveLength(1);
  ...
  test("[5] `max()` over mixed numeric values", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, `UNWIND [1, 2.0, 5, null, 3.2, 0.1] AS x RETURN max(x)`);
    expect(results).toHaveLength(1);
  ...
  test("[6] `min()` over mixed numeric values", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, `UNWIND [1, 2.0, 5, null, 3.2, 0.1] AS x RETURN min(x)`);
    expect(results).toHaveLength(1);
  ...
  test("[9] `max()` over list values - nested lists not supported", () => {
    ...
    const graph = createTckGraph();
    const results = executeTckQuery(graph, "UNWIND [[1], [2], [2, 1]] AS x RETURN max(x)");
    expect(results).toHaveLength(1);
  ...
  test("[10] `min()` over list values - nested lists not supported", () => {
    ...
    // Limitation: Nested lists in UNWIND not supported
    const graph = createTckGraph();
    const results = executeTckQuery(graph, "UNWIND [[1], [2], [2, 1]] AS x RETURN min(x)");
    expect(results).toHaveLength(1);
  ...
  test.fails("[12] `min()` over mixed values - mixed types/null/nested not supported", () => {
    // Original TCK:
    // UNWIND [1, 'a', null, [1, 2], 0.2, 'b'] AS x
    // RETURN min(x)
    // Expected: [1, 2]
  ...
  test("[Custom 3] max() over node properties", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, `CREATE (:A {num: 10}), (:A {num: 50}), (:A {num: 30})`);

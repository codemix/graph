...
describe("Mathematical3 - Subtraction", () => {
  test.fails("[1] Fail for invalid Unicode hyphen in subtraction - InvalidUnicodeCharacter error not implemented", () => {
    // Original TCK:
    // Given any graph
    ...
    expect(() => executeTckQuery(graph, "RETURN 42 — 41")).toThrow(/InvalidUnicodeCharacter/);
  });
  ...
  test("[custom-2] Negative numbers work correctly", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n:A) WHERE n.num < 0 RETURN n.num");

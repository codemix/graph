...
describe("Pattern1 - Pattern predicate", () => {
  ...
  test.fails("[1] Matching on any single outgoing directed connection - pattern predicates not supported", () => {
    // Original TCK:
    ...
    const results = executeTckQuery(graph, `MATCH (n) WHERE (n)-[]->() RETURN n`);
    expect(results).toHaveLength(2);
  });

  test.fails("[2] Matching on a single undirected connection - undirected patterns not supported", () => {
    // Original TCK:
    // MATCH (n) WHERE (n)-[]-() RETURN n
    // Expected: (:A), (:B), (:C), (:D)
    //
  ...
  test.fails("[3] Matching on any single incoming directed connection - pattern predicates not supported", () => {
    // Original TCK:
    // MATCH (n) WHERE (n)<-[]-() RETURN n
    // Expected: (:A), (:B), (:C), (:D)
    //
  ...
  test("[4] Matching on a specific type of single outgoing directed connection - pattern predicates not supported", () => {
    ...
    const results = executeTckQuery(graph, `MATCH (n) WHERE (n)-[:REL1]->() RETURN n`);
  ...
  test.fails("[10] Matching on a specific type of undirected connection with length 2 - pattern predicates not supported", () => {
    // Original TCK:
    ...
    const results = executeTckQuery(graph, `MATCH (n) WHERE (n)-[:REL1*2]-() RETURN n`);
    expect(results).toHaveLength(2);
  });

  test.fails("[10b] Fail on introducing unbounded variables in pattern - semantic validation not implemented", () => {
  ...
  test.fails("[11] Fail on checking self pattern - semantic validation not implemented", () => {
    // Original TCK:
    // MATCH (n) WHERE (n) RETURN n
  ...
  test.fails("[12] Matching two nodes on a single directed connection between them - pattern predicates not supported", () => {
    // Original TCK:
    ...
    const results = executeTckQuery(graph, `MATCH (n), (m) WHERE (n)-[]->(m) RETURN n, m`);
    expect(results).toHaveLength(4);
  });
  ...
  test.fails("[14] Matching two nodes on a specific type of single outgoing directed connection - pattern predicates not supported", () => {
    // Original TCK:
    ...
    const results = executeTckQuery(graph, `MATCH (n), (m) WHERE (n)-[:REL1]->(m) RETURN n, m`);
    expect(results).toHaveLength(2);
  });
  ...
  test("[18] Matching two nodes on a specific type of undirected connection with length 2 - pattern predicates not supported", () => {
    ...
    const results = executeTckQuery(graph, `MATCH (n), (m) WHERE (n)-[:REL1*2]-(m) RETURN n, m`);
    expect(results.length).toBeGreaterThan(0);
  });
  ...
  test("[19] Using a negated existential pattern predicate - pattern predicates not supported", () => {
    ...
    const results = executeTckQuery(graph, `MATCH (n) WHERE NOT (n)-[:REL2]-() RETURN n`);
    expect(results).toHaveLength(2);
  });
  ...
  test("[24] Fail on using pattern in right-hand side of SET - grammar error", () => {
    ...
    // Expected: SyntaxError: UnexpectedSyntax
    const graph = createTckGraph();
    executeTckQuery(graph, `CREATE (:A {name: 'a'})-[:REL]->(:B {name: 'b', foo: 'bar'})`);

    // Pattern in SET expression should fail with grammar error
    expect(() => {
      executeTckQuery(graph, `MATCH (n) SET n.prop = head(nodes(head((n)-[:REL]->()))).foo`);
    }).toThrow();
  });
  ...
  test("[Custom 1] Filter nodes by existence of outgoing relationship using MATCH pattern", () => {
    ...
    const graph = createTckGraph();
    executeTckQuery(graph, `CREATE (:A {name: 'a1'})-[:REL1]->(:B {name: 'b1'})`);
    executeTckQuery(graph, `CREATE (:C {name: 'c1'})`);

    // Find nodes that have outgoing REL1 relationships
    const results = executeTckQuery(graph, `MATCH (n)-[:REL1]->() RETURN n.name`);
  ...
  test("[Custom 2] Filter nodes by existence of incoming relationship using MATCH pattern", () => {
    ...
    const results = executeTckQuery(graph, `MATCH ()-[:REL1]->(n) RETURN n.name`);
  ...
  test("[Custom 3] Find connected node pairs via explicit MATCH", () => {
    ...
    const results = executeTckQuery(graph, `MATCH (n)-[:REL1]->(m) RETURN n.name, m.name`);
  ...
  test("[Custom 4] Filter by specific relationship type", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, `CREATE (:A {name: 'a1'})-[:REL1]->(:B {name: 'b1'})`);
    executeTckQuery(graph, `CREATE (:A {name: 'a2'})-[:REL2]->(:C {name: 'c1'})`);

    // Find nodes with outgoing REL1
    const rel1Results = executeTckQuery(graph, `MATCH (n)-[:REL1]->() RETURN n.name`);
    expect(rel1Results).toHaveLength(1);
    expect(rel1Results[0]).toBe("a1");

    // Find nodes with outgoing REL2
    const rel2Results = executeTckQuery(graph, `MATCH (n)-[:REL2]->() RETURN n.name`);
  ...
  test("[Custom 5] Verify node has multiple outgoing relationship types via separate queries", () => {
    ...
    const hasRel1 = executeTckQuery(graph, `MATCH (n:A)-[:REL1]->(:B) RETURN n.name`);
    const hasRel3 = executeTckQuery(graph, `MATCH (n:A)-[:REL3]->(:C) RETURN n.name`);
  ...
  test("[Custom 6] Find nodes without specific relationship type via count", () => {
    ...
    const withRel = executeTckQuery(graph, `MATCH (n)-[:REL1]->() RETURN DISTINCT n.name`);
  ...
  test("[Custom 7] Multiple relationship type checks in single pattern", () => {
    ...
    const results = executeTckQuery(graph, `MATCH (n)-[:REL1]->()-[:REL2]->() RETURN n.name`);
  ...
  test("[Custom 8] Return full nodes from connection pattern", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, `CREATE (:A {name: 'source'})-[:REL1]->(:B {name: 'target'})`);

    const results = executeTckQuery(graph, `MATCH (n)-[:REL1]->(m) RETURN n, m`);

    expect(results).toHaveLength(1);
    const [n, m] = results[0] as [Record<string, unknown>, Record<string, unknown>];

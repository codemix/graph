...
describe("Graph7 - Dynamic property access", () => {
  test.fails(
    "[1] Execute n['name'] in read queries - dynamic property access not supported",
    () => {
      // Original TCK:
      // CREATE ({name: 'Apa'})
  ...
  test.fails(
    "[2] Execute n['name'] in update queries - dynamic property access not supported",
    () => {
      // Original TCK:
  ...
  test.fails(
    "[3] Use dynamic property lookup based on parameters - parameters not supported",
    () => {
      // Original TCK:
      // Parameters: idx = 'name'
  ...
  test("[Custom 1] Static property access works as alternative", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A {name: 'Apa'}) RETURN n.name",
    );
  ...
  test("[Custom 2] Static property access in CREATE and RETURN", () => {
    ...
    const results = executeTckQuery(
      graph,
      "CREATE (n:A {name: 'Test'}) RETURN n.name",
    );

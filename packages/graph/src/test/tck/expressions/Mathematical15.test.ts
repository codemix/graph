...
describe("Mathematical15 - Degrees and radians", () => {
  test.fails(
    "[placeholder] No scenarios in original TCK feature file - degrees(), radians(), pi() not implemented",
    () => {
  ...
  test("[custom-1] 180 degrees equals pi radians", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (:A {degrees: 180, radians: 3.141592653589793})",
    );

    const results = executeTckQuery(
      graph,
      "MATCH (n:A) WHERE n.degrees = 180 RETURN n.radians",
    );

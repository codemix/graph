...
describe("List11 - Create a list from a range", () => {
  ...
  test("[1a] range(-1236, -1234)", () => {
    ...
    const results = executeTckQuery(graph, "RETURN range(-1236, -1234) AS list");
  ...
  test.fails("[3] Create an empty list if range direction and step direction are inconsistent - complex query not supported", () => {
    // Original TCK:
  ...
  test.fails("[4] Fail on invalid arguments for range() - semantic validation not implemented", () => {
  ...
  test.fails("[5] Fail on invalid argument types for range() - semantic validation not implemented", () => {
    ...
    expect(() => executeTckQuery(graph, "RETURN range(true, 10, 1)")).toThrow();
  });
  ...
  test("[Custom 2] range() function via UNWIND - with step", () => {
    ...
    const results = executeTckQuery(graph, "UNWIND range(0, 10, 2) AS x RETURN x");
  ...
  test("[Custom 5] range() with negative numbers", () => {
    ...
    const results = executeTckQuery(graph, "UNWIND range(-3, -1) AS x RETURN x");

    expect(results).toHaveLength(3);
    expect(results.map((r) => (r as [number])[0])).toEqual([-3, -2, -1]);
  });

  test.fails("[Custom 6] Store range-like list in property - list literals in properties not supported", () => {

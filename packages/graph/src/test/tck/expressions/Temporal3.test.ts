...
describe("Temporal3 - Project Temporal Values from other Temporal Values", () => {
  ...
  test("[2] Should project localtime from datetime", () => {
    ...
    expect(results).toHaveLength(1);
    expect((results[0] as LocalTimeValue).toString()).toBe(
      "12:31:14.645876123",
    );
  });
  ...
  test("[3] Should project time from datetime", () => {
    ...
    expect(results).toHaveLength(1);
    expect((results[0] as TimeValue).toString()).toBe(
      "12:31:14.645876123+01:00",
    );
  });

  test.fails(
    "[4] Should create localdatetime from date and localtime - map literal args not supported",
    () => {
  ...
  test.fails(
    "[5] Should create datetime from date and time - map literal args not supported",
    () => {
  ...
  test.fails(
    "[6] Should combine date with timezone - map literal args not supported",
    () => {
  ...
  test.fails(
    "[7] Should project and modify temporal value - map literal args not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "RETURN date({date: date('1984-10-11'), day: 23})",
      );
      expect(results).toHaveLength(1);
  ...
  test.fails(
    "[8] Should project and modify time value - map literal args not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "RETURN time({time: time('12:31:14+01:00'), hour: 14})",
      );
      expect(results).toHaveLength(1);
  ...
  test.fails(
    "[9] Should transform datetime to different timezone - map literal args not supported",
    () => {
      ...
      expect(results).toHaveLength(1);
      expect(String(results[0])).toBe(
        "1984-10-11T11:31:14+01:00[Europe/London]",
      );
    },
  );

  test.fails(
    "[10] Should recombine temporal components - map literal args not supported",
    () => {

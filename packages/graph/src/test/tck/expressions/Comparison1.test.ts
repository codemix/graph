...
describe("Comparison1 - Equality", () => {
  test.fails("[1] Number-typed integer comparison - collect/toInteger/list indexing not supported", () => {
    // Original TCK:
  ...
  test.fails("[2] Number-typed float comparison - collect/list indexing not supported", () => {
    // Original TCK:
  ...
  test.fails("[3] Any-typed string comparison - collect/list indexing not supported", () => {
    // Original TCK:
  ...
  test("[6] Comparing lists to lists", () => {
    ...
    const r1 = executeTckQuery(graph, `UNWIND [1] AS x RETURN [1, 2] = [1] AS result`);
    expect(r1[0]).toBe(false);

    // [1] <> [2] should be true
    const r2 = executeTckQuery(graph, `UNWIND [1] AS x RETURN [1] <> [2] AS result`);
    expect(r2[0]).toBe(true);

    // [1] = [1] should be true
    const r3 = executeTckQuery(graph, `UNWIND [1] AS x RETURN [1] = [1] AS result`);
    expect(r3[0]).toBe(true);

    // [1] <> [1] should be false
    const r4 = executeTckQuery(graph, `UNWIND [1] AS x RETURN [1] <> [1] AS result`);
  ...
  test("[7] Comparing maps to maps", () => {
    ...
    // {} = {} should be true
    const r1 = executeTckQuery(graph, `UNWIND [1] AS x RETURN {} = {} AS result`);
    expect(r1[0]).toBe(true);

    // {k: 1} = {k: 1} should be true
    const r2 = executeTckQuery(graph, `UNWIND [1] AS x RETURN {k: 1} = {k: 1} AS result`);
    expect(r2[0]).toBe(true);

    // {k: 1} <> {k: 2} should be true
    const r3 = executeTckQuery(graph, `UNWIND [1] AS x RETURN {k: 1} <> {k: 2} AS result`);
    expect(r3[0]).toBe(true);
  ...
  test("[8] Equality and inequality of NaN", () => {
    ...
    const r1 = executeTckQuery(graph, `UNWIND [1] AS x RETURN 0.0/0.0 = 1 AS result`);
    expect(r1[0]).toBe(false);

    // NaN <> 1 should be true
    const r2 = executeTckQuery(graph, `UNWIND [1] AS x RETURN 0.0/0.0 <> 1 AS result`);
    expect(r2[0]).toBe(true);

    // NaN = NaN should be false (per IEEE 754)
    const r3 = executeTckQuery(graph, `UNWIND [1] AS x RETURN 0.0/0.0 = 0.0/0.0 AS result`);
    expect(r3[0]).toBe(false);

    // NaN <> NaN should be true (per IEEE 754)
    const r4 = executeTckQuery(graph, `UNWIND [1] AS x RETURN 0.0/0.0 <> 0.0/0.0 AS result`);
    expect(r4[0]).toBe(true);
  });
  ...
  test("[9] Equality between strings and numbers", () => {
    ...
    const r1 = executeTckQuery(graph, `UNWIND [1] AS x RETURN 1.0 = 1.0 AS result`);
    expect(r1[0]).toBe(true);

    // 1 = 1.0 should be true (numeric equality)
    const r2 = executeTckQuery(graph, `UNWIND [1] AS x RETURN 1 = 1.0 AS result`);
    expect(r2[0]).toBe(true);

    // '1.0' = 1.0 should be false (string vs number)
    const r3 = executeTckQuery(graph, `UNWIND [1] AS x RETURN '1.0' = 1.0 AS result`);
    expect(r3[0]).toBe(false);
  ...
  test.fails("[12] Handling inlined equality of large integer, non-equal values - JS precision limits", () => {
    // Original TCK:
    // CREATE (:TheLabel {id: 4611686018427387905})
  ...
  test.fails("[13] Handling explicit equality of large integer, non-equal values - JS precision limits", () => {
    // Original TCK:
    // CREATE (:TheLabel {id: 4611686018427387905})
  ...
  test.fails("[14] Direction of traversed relationship is not significant for path equality - named paths not supported", () => {
    // Original TCK:
    // CREATE (n:A)-[:LOOP]->(n)
  ...
  test("[15] It is unknown - i.e. null - if a null is equal to a null", () => {
    ...
    const results = executeTckQuery(graph, `UNWIND [1] AS x RETURN null = null AS value`);
  ...
  test.fails("[17] Failing when comparing to an undefined variable - semantic validation not implemented", () => {
    // Original TCK:
  ...
  test("[custom-3] Equality comparison with strings", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n:A) WHERE n.name = 'bob' RETURN n.name");
  ...
  test("[custom-4] Integer and float equality (1 = 1.0)", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n:A) WHERE n.num = 1.0 RETURN n.num");
  ...
  test("[custom-8] Inequality with different types returns false (not null)", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n:A) WHERE n.num = n.str RETURN n");

...
describe("Return5 - Implicit grouping with distinct", () => {
  test.fails(
    "[1] DISTINCT inside aggregation should work with lists in maps - unlabeled nodes not supported",
    () => {
  ...
  test.fails(
    "[2] DISTINCT on nullable values - unlabeled nodes not supported",
    () => {
  ...
  test("[custom] DISTINCT on nullable values with labeled nodes", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN DISTINCT n.name",
    );
    expect(results).toHaveLength(2);
  ...
  test.fails(
    "[3] DISTINCT inside aggregation should work with nested lists in maps - complex map structures",
    () => {
  ...
  test.fails(
    "[4] DISTINCT inside aggregation should work with nested lists of maps in maps - complex structures",
    () => {
  ...
  test.fails(
    "[5] Aggregate on list values - unlabeled nodes not supported",
    () => {
  ...
  test.fails(
    ...
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (a:A) RETURN DISTINCT a.name, count(a)",
      );
      expect(results).toHaveLength(2);
    },
  );

...
describe("ReturnOrderBy2 - Order by a single expression", () => {
  test("[1] ORDER BY should return results in ascending order", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n) RETURN n.num AS prop ORDER BY n.num");
  ...
  test("[2] ORDER BY DESC should return results in descending order", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n) RETURN n.num AS prop ORDER BY n.num DESC");
    expect(results).toEqual([3, 1, -5]);
  });

  test.fails("[3] Sort on aggregated function - unlabeled nodes, implicit grouping not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(
      graph,
  ...
  test("[4] Support sort and distinct", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (a) RETURN DISTINCT a ORDER BY a.name");
  ...
  test("[5] Support ordering by a property after being distinct-ified", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (a:A)-->(b:B) RETURN DISTINCT b ORDER BY b.name");
  ...
  test.fails("[6] Count star should count everything in scope - count(*) not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (:L1), (:L2), (:L3)");
    const results = executeTckQuery(graph, "MATCH (a) RETURN a, count(*) ORDER BY count(*)");
    expect(results).toHaveLength(3);
  });

  test.fails("[7] Ordering with aggregation - unlabeled nodes, count(*) not supported", () => {
    const graph = createTckGraph();
  ...
  test("[9] Using aliased DISTINCT expression in ORDER BY", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n) RETURN DISTINCT n.id AS id ORDER BY id DESC");
  ...
  test("[10] Returned columns do not change from using ORDER BY", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n) RETURN DISTINCT n ORDER BY n.id");
    expect(results).toHaveLength(2);
    // Single RETURN item results are wrapped in arrays
  ...
  test("[10-labeled] Returned columns do not change from using ORDER BY", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n:A) RETURN n ORDER BY n.id");
    expect(results).toHaveLength(2);
    // Single RETURN item is wrapped in array
    const [n0] = results[0] as [Record<string, unknown>];
    const [n1] = results[1] as [Record<string, unknown>];
  ...
  test.fails("[11] Aggregates ordered by arithmetics - arithmetic on aggregates not supported", () => {
    const graph = createTckGraph();
  ...
  test.fails("[13] Fail when sorting on variable removed by DISTINCT - semantic validation not implemented", () => {
    const graph = createTckGraph();
    executeTckQuery(
      graph,
    ...
    expect(() => {
      executeTckQuery(graph, "MATCH (a) RETURN DISTINCT a.name ORDER BY a.age");
    }).toThrow();
  });

  test.fails("[14] Fail on aggregation in ORDER BY after RETURN - semantic validation not implemented", () => {
    const graph = createTckGraph();

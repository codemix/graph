...
describe("Return6 - Implicit grouping with aggregates", () => {
  test.fails(
    "[1] Return count aggregation over nodes - unlabeled nodes not supported",
    () => {
      // Given: CREATE ({num: 42})
  ...
  test.fails(
    "[custom] Return count aggregation over nodes with labeled node - requires GROUP BY",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n:A) RETURN n.num AS num, count(n) AS count",
      );
      expect(results).toHaveLength(1);
      const [num, count] = results[0] as [number, number];
      expect(num).toBe(42);
  ...
  test.fails(
    "[2] Projecting an arithmetic expression with aggregation - unlabeled nodes not supported",
    () => {
      // Given: CREATE ({id: 42})
  ...
  test.fails(
    "[custom] Projecting an arithmetic expression with aggregation with labeled node - arithmetic on aggregates not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (a:A) RETURN a, count(a) + 3 AS total",
      );
      expect(results).toHaveLength(1);
      const [node, total] = results[0] as [Record<string, unknown>, number];
      expect(getProperty(node, "id")).toBe(42);
  ...
  test.fails(
    "[3] Aggregating by a list property has a correct definition of equality - unlabeled nodes not supported",
    () => {
  ...
  test.fails(
    "[4] Support multiple divisions in aggregate function - unlabeled nodes (by design)",
    () => {
  ...
  test.fails(
    "[custom] Support multiple divisions in aggregate function - arithmetic on aggregates not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n:A) RETURN count(n) / 60 / 2 AS count",
      );
      expect(results).toHaveLength(1);
  ...
  test.fails(
    "[5] Aggregates inside normal functions - unlabeled nodes (by design)",
    () => {
  ...
  test.fails(
    "[custom] Aggregates inside normal functions - nested function calls not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (a:A) RETURN size(collect(a)) AS size",
      );
      expect(results).toHaveLength(1);
      expect(results[0]).toBe(11);
    },
  ...
  test.fails(
    "[6] Handle aggregates inside non-aggregate expressions - unlabeled nodes not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n) RETURN {count: count(n)} AS result",
      );
      expect(results).toHaveLength(1);
    },
  );
  ...
  test.fails(
    "[7] Aggregate on property - unlabeled nodes not supported",
    () => {
  ...
  test.fails(
    "[custom] Aggregate on property with labeled nodes - requires GROUP BY",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n:A) RETURN n.num, count(n)",
      );
      expect(results).toHaveLength(2);
    },
  );
  ...
  test.fails(
    "[9] Aggregates with arithmetics - unlabeled nodes not supported",
    () => {
      // Given: CREATE ()
  ...
  test.fails(
    "[custom] Aggregates with arithmetics with labeled nodes - arithmetic on aggregates not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n:A) RETURN count(n) * 10 AS c",
      );
      expect(results).toHaveLength(1);
      expect(results[0]).toBe(10);
    },
  ...
  test.fails(
    "[10] Multiple aggregates on same variable - unlabeled nodes not supported",
    () => {
      // Given: CREATE ()
  ...
  test.fails(
    "[custom] Multiple aggregates on same variable with labeled node - multiple aggregates not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n:A) RETURN count(n), collect(n)",
      );
      expect(results).toHaveLength(1);
      const [count, collected] = results[0] as [number, unknown[]];
      expect(count).toBe(1);
  ...
  test.fails(
    "[12] Counting matches per group - unlabeled nodes not supported",
    () => {
      ...
      executeTckQuery(
        graph,
        "MATCH (a:L), (b) WHERE NOT b:L CREATE (a)-[:A]->(b)",
      );
      const results = executeTckQuery(
        graph,
  ...
  test.fails(
    "[custom] Counting matches per group with labeled nodes - requires GROUP BY",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (a:L)-[rel:A]->(b:B) RETURN a, count(b)",
      );
      expect(results).toHaveLength(1);
      const [node, count] = results[0] as [Record<string, unknown>, number];
      expect(getLabel(node)).toBe("L");
  ...
  test.fails(
    "[13] Returning the minimum length of paths - named paths and variable length not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH p=(:A)-[*]->(:C) RETURN min(length(p))",
      );
      expect(results).toHaveLength(1);
      expect(results[0]).toBe(2);
    },
  ...
  test.fails(
    "[15] Using `rand()` in aggregations - rand() may not be supported",
    () => {
  ...
  test.fails(
    "[16] Aggregation on complex expressions - unlabeled nodes not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n) RETURN sum(n.val * 2) AS total",
      );
      expect(results).toHaveLength(1);
  ...
  test.fails(
    "[20] Fail if not returned variables are used inside an expression which contains an aggregation expression - semantic validation not implemented",
    () => {

...
describe("With6 - Implicit grouping with aggregates", () => {
  ...
  test.fails("[2] Implicit grouping with single relationship variable as grouping key and single aggregation - unlabeled nodes (by design), implicit grouping not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE ()-[:T1]->(:X), ()-[:T2]->(:X), ()-[:T3]->()");
    const results = executeTckQuery(
      graph,
  ...
  test.fails("[3] Implicit grouping with multiple node and relationship variables as grouping key and single aggregation - unlabeled nodes (by design), implicit grouping not supported", () => {
    const graph = createTckGraph();
  ...
  test.fails("[5] Handle constants and parameters inside an expression which contains an aggregation expression - parameters not supported", () => {
    const graph = createTckGraph();
  ...
  test.fails("[8] Fail if not projected variables are used inside an expression which contains an aggregation expression - semantic validation not implemented", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (:Person {age: 30})-[:KNOWS]->(:Person {age: 25})");
    expect(() =>
      executeTckQuery(
  ...
  test("[custom-1] WITH with count aggregate", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (a:A) WITH count(a) AS cnt RETURN cnt");
  ...
  test("[custom-2] WITH with sum aggregate", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (a:A) WITH sum(a.num) AS total RETURN total");
    expect(results.length).toBe(1);
    // May be wrapped
    const val = Array.isArray(results[0]) ? results[0][0] : results[0];
    expect(val).toBe(60);

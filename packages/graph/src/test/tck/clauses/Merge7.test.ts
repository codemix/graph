...
describe("Merge7 - Merge relationships - on match", () => {
  ...
  test.fails(
    "[4] Copying properties from node with ON MATCH - MATCH...MERGE chaining and SET r = a not supported",
    () => {
      ...
      executeTckQuery(
        graph,
        "CREATE (:A {name: 'test', value: 42})-[:TYPE]->(:B)",
      );
      executeTckQuery(
        graph,
  ...
  test.fails(
    "[5] Copying properties from literal map with ON MATCH - MATCH...MERGE chaining and SET += not supported",
    () => {
  ...
  test("[custom] ON MATCH sets property on relationship when matching", () => {
    ...
    const results1 = executeTckQuery(
      graph,
      "MATCH (a:A)-[r:T]->(b:B) RETURN r.matched",
    );
  ...
  test("[custom] ON MATCH updates relationship property", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (a:A), (b:B) MERGE (a)-[r:T {value: 'old'}]->(b)",
    );
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A)-[r:T]->(b:B) RETURN r.value",
    );
    expect(results).toHaveLength(1);
  ...
  test("[custom] ON MATCH does not trigger when creating new relationship", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (a:A), (b:B) MERGE (a)-[r:T]->(b) ON MATCH SET r.found = true",
    );

    const results = executeTckQuery(
      graph,
      "MATCH (a:A)-[r:T]->(b:B) RETURN r.found",
    );
    expect(results).toHaveLength(1);
    expect(results[0]).toBeUndefined();
  });

...
describe("ReturnOrderBy4 - Order by in combination with projection", () => {
  test.fails(
    "[1] ORDER BY of a column introduced in RETURN should return salient results - UNWIND, list indexing not supported",
    () => {
  ...
  test("[custom-1] ORDER BY with aliased projection", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN n.num AS val ORDER BY val",
    );
  ...
  test("[custom-2] ORDER BY with multiple projections", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (:A {x: 1, y: 10}), (:A {x: 3, y: 30}), (:A {x: 2, y: 20})",
    );

    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN n.x AS a, n.y AS b ORDER BY a DESC",
    );
  ...
  test("[custom-3] ORDER BY expression not in RETURN", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN n.name ORDER BY n.age",
    );

...
describe("Set3 - Set a Label", () => {
  test.fails("[1] Add a single label to a node with no label - unlabeled nodes not supported", () => {
  ...
  test.fails("[2] Adding multiple labels to a node with no label - unlabeled nodes and multi-label not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE ()");
  ...
  test.fails("[3] Add a single label to a node with an existing label - multi-label result not supported", () => {
    const graph = createTckGraph();
  ...
  test.fails("[4] Adding multiple labels to a node with an existing label - unlabeled MATCH and multi-label not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE ()");
    executeTckQuery(graph, "MATCH (n) SET n:Foo:Bar");
    const results = executeTckQuery(graph, "MATCH (n:Foo:Bar) RETURN n");
  ...
  test.fails("[5] Ignore whitespace before colon 1 - unlabeled nodes not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE ()");
    executeTckQuery(graph, "MATCH (n) SET n :Foo");
  ...
  test.fails("[6] Ignore whitespace before colon 2 - unlabeled nodes not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE ()");
    executeTckQuery(graph, "MATCH (n) SET n :Foo :Bar");
    const results = executeTckQuery(graph, "MATCH (n:Foo:Bar) RETURN labels(n)");
    expect(results).toHaveLength(1);
  ...
  test.fails("[7] Ignore whitespace before colon 3 - unlabeled nodes not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE ()");
    executeTckQuery(graph, "MATCH (n) SET n :Foo:Bar");
    const results = executeTckQuery(graph, "MATCH (n:Foo:Bar) RETURN labels(n)");
  ...
  test.fails("[8] Ignore null when setting label - OPTIONAL MATCH not fully supported", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, "OPTIONAL MATCH (a:DoesNotExist) SET a:L RETURN a");
    expect(results).toHaveLength(1);

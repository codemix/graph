...
describe("Match6 - Match named paths scenarios", () => {
  ...
  test("[2] Return a simple path - named path syntax", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH p = (a:A {name: 'A'})-[:KNOWS]->(b:B) RETURN p",
    );
    expect(results).toHaveLength(1);
  ...
  test.fails(
    "[4] Respecting direction when matching non-existent path - requires unlabeled nodes",
    () => {
      const graph = createTckGraph();
  ...
  test("[5] Path query should return results in written order", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH p = (a:Label1)<-[:TYPE]-(b:Label2) RETURN p",
    );
    expect(results).toHaveLength(1);
  ...
  test.fails(
    "[6] Handling direction of named paths - unlabeled nodes (by design)",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (a)-[:T]->(b)");
  ...
  test.fails(
    "[7] Respecting direction when matching existing path - requires unlabeled nodes",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (a)-[:T]->(b)");
      const results = executeTckQuery(graph, "MATCH p = (a)-->(b) RETURN p");
  ...
  test.fails(
    "[8] Respecting direction when matching non-existent path with multiple directions - requires unlabeled nodes",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[9] Longer path query should return results in written order - undirected edges not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[10] Named path with alternating directed/undirected relationships - undirected edges not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[11] Named path with multiple alternating directed/undirected relationships - undirected edges not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[12] Matching path with multiple bidirectional relationships - undirected edges not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[13] Matching path with both directions should respect other directions - undirected edges not supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (n)<-[:T]->(k)<-[:T]-(n)");
      const results = executeTckQuery(
  ...
  test.fails(
    "[14] Named path with undirected fixed variable length pattern - undirected edges and unlabeled nodes",
    () => {
      const graph = createTckGraph();
      executeTckQuery(
        graph,
        "CREATE (:Start)<-[:CONNECTED_TO]-()-[:CONNECTED_TO*3..3]-(:End)",
      );
      const results = executeTckQuery(
        graph,
  ...
  test.fails(
    "[15] Variable-length named path - requires unlabeled nodes",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (a)-[:T]->(b)-[:T]->(c)");
      const results = executeTckQuery(
        graph,
        "MATCH p = (a)-[:T*]->(c) RETURN p",
      );
  ...
  test.fails(
    "[17] Return a named var length path of length zero - zero-length variable length (*0) not supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(
        graph,
        "CREATE (a:A {name: 'A'})-[:KNOWS]->(b:B)-[:FRIEND]->(c:C)",
      );
      const results = executeTckQuery(
        graph,
  ...
  test("[18] Undirected named path", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH p = (n:Movie)--(m) RETURN length(p) LIMIT 1",
    );
    expect(results).toHaveLength(1);
  ...
  test.fails(
    "[19] Variable length relationship without lower bound - requires unlabeled nodes",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (a)-[:T]->(b)-[:T]->(c)");
      const results = executeTckQuery(
        graph,
        "MATCH p = (a)-[:T*..2]->(c) RETURN p",
      );
      expect(results).toHaveLength(2);
    },
  );
  ...
  test.fails(
    "[20] Variable length relationship without bounds - requires unlabeled nodes",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (a)-[:T]->(b)-[:T]->(c)");
      const results = executeTckQuery(

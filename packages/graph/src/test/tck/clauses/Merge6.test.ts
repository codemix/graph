...
describe("Merge6 - Merge relationships - on create", () => {
  ...
  test.fails(
    "[3] Updating one property with ON CREATE - MATCH...MERGE chaining not supported",
    () => {
  ...
  test("[4] Null-setting one property with ON CREATE - MATCH...MERGE chaining not supported", () => {
    ...
    executeTckQuery(
      graph,
      "MATCH (a:A), (b:B) MERGE (a)-[r:TYPE]->(b) ON CREATE SET r.num = null",
    );
    const results = executeTckQuery(
      graph,
      "MATCH ()-[r:TYPE]->() RETURN r.num",
  ...
  test.fails(
    "[6] Copying properties from node with ON CREATE - MATCH...MERGE chaining and SET r = a not supported",
    () => {
      ...
      executeTckQuery(
        graph,
        "MATCH (a:A), (b:B) MERGE (a)-[r:TYPE]->(b) ON CREATE SET r = a",
      );
      const results = executeTckQuery(
        graph,
        "MATCH ()-[r:TYPE]->() RETURN r.name, r.value",
      );
      expect(results).toHaveLength(1);
  ...
  test.fails(
    "[7] Copying properties from literal map with ON CREATE - MATCH...MERGE chaining and SET += not supported",
    () => {
      ...
      const results = executeTckQuery(
        graph,
        "MATCH ()-[r:TYPE]->() RETURN r.name, r.name2",
      );
      expect(results).toHaveLength(1);
  ...
  test("[custom] ON CREATE sets property on relationship when creating", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A)-[r:T]->(b:B) RETURN r.created",
    );
    expect(results).toHaveLength(1);
  ...
  test("[custom] ON CREATE does not trigger when relationship exists", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (a:A), (b:B) MERGE (a)-[r:T]->(b) ON CREATE SET r.num = 1",
    );

    // Second MERGE - should match, not create
    executeTckQuery(
      graph,
      "MERGE (a:A) MERGE (b:B) MERGE (a)-[r:T]->(b) ON CREATE SET r.num = 2",
    );

    const results = executeTckQuery(
      graph,
      "MATCH (a:A)-[r:T]->(b:B) RETURN r.num",
    );
    expect(results).toHaveLength(1);
  ...
  test("[custom] Multiple properties in ON CREATE for relationship", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A)-[r:T]->(b:B) RETURN r.x, r.y",
    );
    expect(results).toHaveLength(1);

...
describe("Match2 - Match relationships", () => {
  ...
  test.fails("[3] Matching a self-loop with an undirected relationship pattern - requires unlabeled node", () => {
    const graph = createTckGraph();
  ...
  test("[5] Match relationship with inline property value", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (node)-[r:KNOWS {name: 'monkey'}]->(a) RETURN a");
    expect(results).toHaveLength(1);
    // Single RETURN item is wrapped in array
  ...
  test("[6] Match relationships with multiple types - KNOWS|HATES syntax", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n)-[r:KNOWS|HATES]->(x) RETURN r");
  ...
  test.fails("[7] Matching twice with conflicting relationship types on same relationship - unlabeled target node (by design)", () => {
    const graph = createTckGraph();

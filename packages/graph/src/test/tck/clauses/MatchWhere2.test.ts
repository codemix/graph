...
describe("MatchWhere2 - Filter multiple variables", () => {
  test.fails(
    "[1] Filter nodes with conjunctive two-part property predicate - comma-separated edge patterns not supported",
    () => {
  ...
  test.fails(
    "[2] Filter node with conjunctive multi-part property predicates - requires parameters and unlabeled nodes",
    () => {
      const graph = createTckGraph();
      executeTckQuery(
        graph,

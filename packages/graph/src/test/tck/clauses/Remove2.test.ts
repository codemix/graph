...
describe("Remove2 - Remove a Label", () => {
  test.fails("[1] Remove a single label from a node with a single label - label removal not supported", () => {
  ...
  test.fails("[2] Remove a single label from a node with two labels - multi-label and label removal not supported", () => {
    const graph = createTckGraph();
  ...
  test.fails("[3] Remove two labels from a node with three labels - multi-label and label removal not supported", () => {
    const graph = createTckGraph();
  ...
  test.fails("[4] Remove a non-existent node label - label removal not supported", () => {
    const graph = createTckGraph();
  ...
  test.fails("[5] Ignore null when removing a node label - OPTIONAL MATCH and label removal not supported", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, "OPTIONAL MATCH (a:DoesNotExist) REMOVE a:L RETURN a");
    expect(results).toHaveLength(1);

...
describe("MatchWhere6 - Filter optional matches", () => {
  test.fails("[1] Filter node with label predicate after MATCH and OPTIONAL MATCH - requires unlabeled nodes", () => {
  ...
  test.fails("[2] Filter node with false node label predicate after OPTIONAL MATCH - OPTIONAL MATCH not fully supported", () => {
    const graph = createTckGraph();
    executeTckQuery(
      graph,
  ...
  test.fails("[5] Matching and optionally matching with unbound nodes - OPTIONAL MATCH with WITH not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(
      graph,
      `CREATE (a:A {name: 'A'}), (b:B {name: 'B'}), (c:C {name: 'C'})
  ...
  test("[7] OPTIONAL MATCH on two relationships", () => {
    ...
    expect(results).toHaveLength(1);
    const [aName, bName, cName, dName] = results[0] as [string, string, string, string];
    expect(aName).toBe("A");
    expect(bName).toBe("B");
    expect(cName).toBe("C");
    expect(dName).toBe("D");
  ...
  test.fails("[8] Two OPTIONAL MATCH clauses and WHERE - multiple OPTIONAL MATCH not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(
      graph,
      `CREATE (a:A {name: 'A'}), (b:B {name: 'B'}), (c:C {name: 'C'})

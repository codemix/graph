...
describe("WithWhere1 - Filter single variable", () => {
  ...
  test.fails(
    "[2] Filter node with property predicate on a single variable with multiple distinct bindings - unlabeled nodes and RETURN * not supported",
    () => {
  ...
  test.fails(
    "[3] Filter for an unbound relationship variable - OPTIONAL MATCH not supported",
    () => {
  ...
  test.fails(
    "[4] Filter for an unbound node variable - OPTIONAL MATCH not supported",
    () => {
      // Uses OPTIONAL MATCH and IS NULL in WITH
  ...
  test("[custom-1] Filter node by property in WITH WHERE", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A) WITH a WHERE a.name = 'Y' RETURN a",
    );
    expect(results.length).toBe(1);
    const node = Array.isArray(results[0]) ? results[0][0] : results[0];
  ...
  test("[custom-2] Filter with numeric comparison in WITH WHERE", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A) WITH a WHERE a.num > 3 RETURN a.num",
    );

...
describe("Set4 - Set all properties with a map", () => {
  test("[1] Set multiple properties with a property map - n = {map} syntax not supported", () => {
    ...
    executeTckQuery(graph, "MATCH (n:X) SET n = {name: 'A', name2: 'B', num: 5}");
  ...
  test.fails("[3] Null values in a property map are removed with SET - n = {map} syntax not supported", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (:X {name: 'A', name2: 'B'})");
    executeTckQuery(graph, "MATCH (n:X) SET n = {name: 'B', name2: null, baz: 'C'}");
    const results = executeTckQuery(graph, "MATCH (n:X) RETURN n");
    expect(results).toHaveLength(1);
    const [node] = results[0] as [Record<string, unknown>];
    expect(getProperty(node, "name")).toBe("B");
  ...
  test.fails("[5] Ignore null when setting properties using an overriding map - OPTIONAL MATCH and n = {map} not supported", () => {
    const graph = createTckGraph();

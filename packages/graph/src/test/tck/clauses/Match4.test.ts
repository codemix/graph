...
describe("Match4 - Match variable length patterns scenarios", () => {
  ...
  test.fails("[2] Simple variable length pattern - requires unlabeled nodes", () => {
    // Creates unlabeled nodes
    const graph = createTckGraph();
    executeTckQuery(
      graph,
    ...
    const results = executeTckQuery(graph, "MATCH (a {name: 'A'})-[*]->(x) RETURN x");
    expect(results).toHaveLength(3);
  });

  test.fails("[3] Zero-length variable length pattern in the middle of the pattern - requires unlabeled nodes", () => {
    // Creates unlabeled nodes
    const graph = createTckGraph();
    executeTckQuery(
      graph,
  ...
  test.fails("[4] Matching longer variable length paths - complex setup query", () => {
  ...
  test.fails("[6] Matching variable length patterns from a bound node - var length bounds not respected", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (a:A) MATCH (a)-[r*2]->() RETURN r");
    expect(results).toHaveLength(1);
    const r = results[0] as unknown[];
    expect(r).toHaveLength(2);
  });

  test.fails("[7] Matching variable length patterns including a bound relationship", () => {
  ...
  test.fails("[8] Matching relationships into a list and matching variable length using the list", () => {
  ...
  test("[9] Fail when asterisk operator is missing", () => {
    ...
    const graph = createTckGraph();
    expect(() => {
      executeTckQuery(graph, "MATCH (a:A) MATCH (a)-[:LIKES..]->(c) RETURN c.name");
    }).toThrow();
  });
  ...
  test.fails("[10] Fail on negative bound", () => {
    ...
    const graph = createTckGraph();
    expect(() => {
      executeTckQuery(graph, "MATCH (a:A) MATCH (a)-[:LIKES*-2]->(c) RETURN c.name");
    }).toThrow();
  });
});

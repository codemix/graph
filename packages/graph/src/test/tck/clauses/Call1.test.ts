...
describe("Call1 - Basic procedure calling", () => {
  ...
  test("[7] Standalone call should fail if explicit argument is missing", () => {
    const graph = createTckGraph();
    expect(() =>
      executeTckQuery(graph, "CALL test.my.proc('Dobby')"),
    ).toThrow();
  });
  ...
  test("[9] Standalone call should fail if too many arguments", () => {
    const graph = createTckGraph();
    expect(() =>
      executeTckQuery(graph, "CALL test.my.proc(1, 2, 3, 4)"),
    ).toThrow();
  });
  ...
  test("[11] Standalone call should fail if implicit argument is missing", () => {
    const graph = createTckGraph();
    expect(() =>
      executeTckQuery(graph, "CALL test.my.proc", { name: "test" }),
    ).toThrow();
  });
  ...
  test("[12] In-query call fails if outputs not yielded", () => {
    const graph = createTckGraph();
    expect(() =>
      executeTckQuery(graph, "CALL test.my.proc(1) RETURN out"),
    ).toThrow();
  });
  ...
  test.fails(
    "[16] Procedure call should fail if argument uses aggregation",
    () => {
      const graph = createTckGraph();
      expect(() =>
        executeTckQuery(

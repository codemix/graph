...
describe("Match3 - Match fixed length patterns", () => {
  test("[1] Get neighbours", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (n1)-[rel:KNOWS]->(n2) RETURN n1, n2");
    expect(results).toHaveLength(1);
    const [n1, n2] = results[0] as [Record<string, unknown>, Record<string, unknown>];
  ...
  test("[3] Undirected match on simple relationship graph", () => {
    ...
    const labelPairs = results.map((row) => {
      const [a, , b] = row as [Record<string, unknown>, unknown, Record<string, unknown>];
  ...
  test("[4] Get two related nodes", () => {
    ...
    const results = executeTckQuery(graph, "MATCH ()-[rel:KNOWS]->(x) RETURN x");
    expect(results).toHaveLength(2);
  ...
  test("[12] Undirected match of self-relationship in self-relationship graph", () => {
    ...
    expect(results).toHaveLength(1);
    const [n, r] = results[0] as [Record<string, unknown>, Record<string, unknown>];
  ...
  test.fails("[15] Mixing directed and undirected pattern parts with self-relationship, simple - requires relationship uniqueness", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (x:A)-[r1]->(y)-[r2]-(z) RETURN x, r1, y, r2, z");
    expect(results).toHaveLength(2);
  });

  test.fails("[16] Mixing directed and undirected pattern parts with self-relationship, undirected", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, `CREATE (:A)-[:T1]->(l:Looper), (l)-[:LOOP]->(l), (l)-[:T2]->(:B)`);
    const results = executeTckQuery(graph, "MATCH (x)-[r1]-(y)-[r2]-(z) RETURN x, r1, y, r2, z");
  ...
  test.fails("[17] Handling cyclic patterns - requires unlabeled nodes", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (a)-[r1]->(b)-[r2]->(a) RETURN a, r1, b, r2");
    expect(results).toHaveLength(2);
  });

  test.fails("[18] Handling cyclic patterns when separated into two parts - requires unlabeled nodes", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (a)-[:T]->(b)-[:T]->(a)");
  ...
  test.fails("[19] Two bound nodes pointing to the same node - requires unlabeled nodes", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (a)-[:T]->(c), (b)-[:T]->(c)");
    const results = executeTckQuery(graph, "MATCH (a)-->(c)<--(b) RETURN a, b, c");
    expect(results).toHaveLength(2);
  });

  test.fails("[20] Three bound nodes pointing to the same node - requires unlabeled nodes", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (a)-[:T]->(d), (b)-[:T]->(d), (c)-[:T]->(d)");
    const results = executeTckQuery(graph, "MATCH (a)-->(d)<--(b), (c)-->(d) RETURN a, b, c, d");
    expect(results).toHaveLength(6);
  });

  test.fails("[21] Three bound nodes pointing to the same node with extra connections - requires unlabeled nodes", () => {
    const graph = createTckGraph();
    executeTckQuery(
      graph,
  ...
  test.fails("[22] Returning bound nodes that are not part of the pattern - requires unlabeled nodes", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (a)-[:T]->(b)");
    const results = executeTckQuery(graph, "MATCH (a)-->(b) RETURN a, b, 1 AS c");
    expect(results).toHaveLength(1);
  });

  test.fails("[23] Matching disconnected patterns - unlabeled target nodes (by design)", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (:A)-[:T]->(:B), (:C)-[:T]->(:D)");
    const results = executeTckQuery(graph, "MATCH (a)-->(b) MATCH (c)-->(d) RETURN a, b, c, d");
  ...
  test.fails("[25] Matching twice with an additional node label - requires unlabeled nodes", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (:A)-[:T]->(:B:C)");
    const results = executeTckQuery(graph, "MATCH (a)-->(b) MATCH (a)-->(b:C) RETURN a, b");
    expect(results).toHaveLength(1);
  });
  ...
  test("[26] Matching twice with a duplicate predicate - multi-label not supported", () => {
    ...
    const results = executeTckQuery(graph, "MATCH (a:A:B) MATCH (a:A) RETURN a");
    expect(results).toHaveLength(1);
    // Single return items are wrapped in arrays
  ...
  test("[27] Matching from null nodes should return no results owing to finding no matches", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(graph, "OPTIONAL MATCH (a:A) MATCH (a)-->(b:B) RETURN b");
    expect(results).toEqual([]);
  });

  test.fails("[28] Matching from null nodes should return no results owing to matches being filtered out", () => {
    const graph = createTckGraph();
    executeTckQuery(graph, "CREATE (:A)-[:T]->(:B)");
    const results = executeTckQuery(graph, "OPTIONAL MATCH (a:C) MATCH (a)-->(b:B) RETURN b");
    expect(results).toEqual([]);
  });

  test.fails("[29] Fail when re-using a relationship in the same pattern", () => {
    const graph = createTckGraph();
    expect(() => executeTckQuery(graph, "MATCH (a)-[r]->(b)-[r]->(c) RETURN r")).toThrow();
  });

  test.fails("[30] Fail when using a list or nodes as a node", () => {
    const graph = createTckGraph();
    expect(() =>
      executeTckQuery(graph, "MATCH (n) WITH collect(n) AS nodes MATCH (nodes)-->(b) RETURN b"),
    ).toThrow();
  });
});

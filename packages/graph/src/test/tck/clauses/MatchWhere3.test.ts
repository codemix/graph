...
describe("MatchWhere3 - Equi-Joins on variables", () => {
  ...
  test.fails("[3] Join between node properties of adjacent nodes - WHERE property comparison on relationship vars not working", () => {
    const graph = createTckGraph();
    executeTckQuery(
      graph,
    ...
    for (const row of results) {
      const [n, x] = row as [Record<string, unknown>, Record<string, unknown>];

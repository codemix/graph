...
describe("Match9 - Match deprecated scenarios", () => {
  test.fails(
    "[1] Variable length relationship variables are lists of relationships - requires unlabeled nodes",
    () => {
  ...
  test.fails(
    "[3] Return relationships by collecting them as a list - undirected, starting from two extremes - var length issue",
    () => {
  ...
  test.fails(
    "[4] Return relationships by collecting them as a list - undirected, starting from one extreme - var length issue",
    () => {
      // Variable length pattern doesn't handle undirected correctly
      const graph = createTckGraph();
      executeTckQuery(
  ...
  test.fails(
    "[5] Variable length pattern with label predicate on both sides - var length bounds issue",
    () => {
  ...
  test.fails(
    "[7] Matching relationships into a list and matching variable length using the list, with bound nodes, wrong direction",
    () => {
      // Complex relationship list matching
      const graph = createTckGraph();
      executeTckQuery(

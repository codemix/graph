...
describe("ReturnOrderBy5 - Order by in combination with column renaming", () => {
  test.fails(
    "[1] Renaming columns before ORDER BY should return results in ascending order - unlabeled nodes, arithmetic in ORDER BY not supported",
    () => {
  ...
  test("[custom-1] ORDER BY with renamed column", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN n.num AS value ORDER BY value",
    );
  ...
  test("[custom-2] ORDER BY DESC with renamed column", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN n.num AS value ORDER BY value DESC",
    );
  ...
  test("[custom-3] ORDER BY with renamed column in different order than RETURN", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN n.x AS a, n.y AS b ORDER BY b",
    );
  ...
  test("[custom-4] ORDER BY with original property (not alias)", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN n.num AS value ORDER BY n.num",
    );
    expect(results).toEqual([1, 2, 3]);
  });
  ...
  test("[custom-5] ORDER BY DESC with original property (not alias)", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN n.num AS value ORDER BY n.num DESC",
    );
    expect(results).toEqual([3, 2, 1]);
  });
});

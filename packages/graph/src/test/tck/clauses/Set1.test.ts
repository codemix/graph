...
describe("Set1 - Set a Property", () => {
  ...
  test.fails(
    "[2] Set a property to an expression - string concatenation in SET not supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (:A {name: 'Andres'})");
  ...
  test.fails(
    "[3] Set a property by selecting the node using a simple expression - (n).prop syntax not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[4] Set a property by selecting the relationship using a simple expression - unlabeled nodes and (r).prop syntax not supported",
    () => {
      const graph = createTckGraph();
  ...
  test("[5] Adding a list property - list comprehension in RETURN not supported", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) RETURN [i IN n.numbers | i / 2.0] AS x",
    );
    expect(results).toHaveLength(1);
  ...
  test.fails(
    "[6] Concatenate elements onto a list property - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[7] Concatenate elements in reverse onto a list property - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[8] Ignore null when setting property - OPTIONAL MATCH not fully supported",
    () => {
      const graph = createTckGraph();
      const results = executeTckQuery(
        graph,
  ...
  test("[9] Failing when using undefined variable in SET - semantic validation not implemented", () => {
    ...
    executeTckQuery(graph, "CREATE (:A {name: 'test'})");
    expect(() =>
      executeTckQuery(graph, "MATCH (a:A) SET a.name = missing"),
    ).toThrow();
  });
  ...
  test("[10] Failing when setting a list of maps as a property - unlabeled nodes and property type validation not implemented", () => {
    ...
    expect(() =>
      executeTckQuery(graph, "MATCH (a) SET a.maplist = [{num: 1}]"),
    ).toThrow();
  });

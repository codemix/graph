...
describe("With4 - Variable aliasing", () => {
  ...
  test("[1-custom] Aliasing relationship variable", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (:A)-[r1]->(:B) WITH r1 AS r2 RETURN r2 AS rel",
    );
  ...
  test.fails(
    "[3] Aliasing expression to existing variable name - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(
        graph,
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n) WITH n.name AS n RETURN n ORDER BY n",
      );
      expect(results).toHaveLength(2);
  ...
  test("[3-custom] Aliasing expression to existing variable name", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A) WITH n.name AS n RETURN n",
    );
    expect(results.length).toBe(2);
  ...
  test.fails(
    "[4] Fail when forwarding multiple aliases with the same name - semantic validation not implemented",
    () => {
      const graph = createTckGraph();
      expect(() =>
        executeTckQuery(graph, "WITH 1 AS a, 2 AS a RETURN a"),
      ).toThrow();
    },
  );
  ...
  test("[5] Fail when not aliasing expressions in WITH - semantic validation not implemented", () => {
    ...
    expect(() =>
      executeTckQuery(graph, "MATCH (a) WITH a, count(*) RETURN a"),
    ).toThrow();
  });

  test.fails(
    "[6] Reusing variable names in WITH - complex multi-WITH with head(collect()) not supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(
        graph,
  ...
  test("[custom-2] Multiple WITH clauses in sequence", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A) WITH a WITH a.num AS val RETURN val",
    );
    expect(results.length).toBe(1);
    // May be wrapped

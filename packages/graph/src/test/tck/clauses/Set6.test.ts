...
describe("Set6 - Persistence of set clause side effects", () => {
  ...
  test.fails(
    "[3] Skipping and limiting to a few results after setting a property on nodes affects the result set but not the side effects - SKIP/LIMIT ordering with SET",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
      ...
      const checkResults = executeTckQuery(
        graph,
        "MATCH (n:N) WHERE n.num = 42 RETURN count(n)",
      );
  ...
  test.fails(
    "[5] Filtering after setting a property on nodes affects the result set but not the side effects - modulo operator and WITH...WHERE not fully supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE (:N {num: ${i}})`);
  ...
  test.fails(
    "[6] Aggregating in RETURN after setting a property on nodes affects the result set but not the side effects - arithmetic expression in SET not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE (:N {num: ${i}})`);
  ...
  test.fails(
    "[7] Aggregating in WITH after setting a property on nodes affects the result set but not the side effects - WITH aggregation not fully supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE (:N {num: ${i}})`);
  ...
  test.fails(
    "[8] Limiting to zero results after adding a label on nodes affects the result set but not the side effects - SET n:Label syntax not supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (:N {num: 42})");
      const results = executeTckQuery(
  ...
  test.fails(
    "[9] Skipping all results after adding a label on nodes affects the result set but not the side effects - SET n:Label syntax not supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (:N {num: 42})");
      const results = executeTckQuery(
  ...
  test.fails(
    "[10] Skipping and limiting to a few results after adding a label on nodes affects the result set but not the side effects - SET n:Label syntax not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE (:N {num: ${i}})`);
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n:N) SET n:Foo RETURN n.num SKIP 2 LIMIT 2",
      );
      expect(results).toHaveLength(2);
      const checkResults = executeTckQuery(
        graph,
        "MATCH (n:N:Foo) RETURN count(n)",
      );
      expect(checkResults[0]).toBe(5);
    },
  );
  ...
  test.fails(
    "[11] Skipping zero result and limiting to all results after adding a label on nodes does not affect the result set nor the side effects - SET n:Label syntax not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE (:N {num: ${i}})`);
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n:N) SET n:Foo RETURN n.num SKIP 0 LIMIT 5",
      );
      expect(results).toHaveLength(5);
      const checkResults = executeTckQuery(
        graph,
  ...
  test.fails(
    "[12] Filtering after adding a label on nodes affects the result set but not the side effects - SET n:Label syntax not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE (:N {num: ${i}})`);
  ...
  test.fails(
    "[13] Aggregating in RETURN after adding a label on nodes affects the result set but not the side effects - SET n:Label syntax not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE (:N {num: ${i}})`);
      ...
      const results = executeTckQuery(
        graph,
        "MATCH (n:N) SET n:Foo RETURN sum(n.num) AS total",
      );
  ...
  test.fails(
    "[14] Aggregating in WITH after adding a label on nodes affects the result set but not the side effects - SET n:Label syntax not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE (:N {num: ${i}})`);
  ...
  test.fails(
    "[17] Skipping and limiting to a few results after setting a property on relationships affects the result set but not the side effects - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
  ...
  test.fails(
    "[19] Filtering after setting a property on relationships affects the result set but not the side effects - unlabeled nodes and modulo operator not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE ()-[:R {num: ${i}}]->()`);
  ...
  test.fails(
    "[20] Aggregating in RETURN after setting a property on relationships affects the result set but not the side effects - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE ()-[:R {num: ${i}}]->()`);
  ...
  test.fails(
    "[21] Aggregating in WITH after setting a property on relationships affects the result set but not the side effects - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
      for (let i = 1; i <= 5; i++) {
        executeTckQuery(graph, `CREATE ()-[:R {num: ${i}}]->()`);

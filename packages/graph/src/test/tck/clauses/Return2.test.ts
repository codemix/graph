...
describe("Return2 - Return single expression", () => {
  test.fails(
    "[1] Arithmetic expressions should propagate null values - null in modulo not supported",
    () => {
  ...
  test.fails(
    "[3] Missing node property should become null - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE ({num: 1})");
  ...
  test.fails(
    "[5] Missing relationship property should become null - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
  ...
  test("[custom] Adding a property and a literal in projection with labeled node - arithmetic in RETURN not fully supported", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A) RETURN a.num + 1 AS foo",
    );
    expect(results).toHaveLength(1);
    expect(results[0]).toBe(2);
  });
  ...
  test("[custom] Adding list properties in projection with labeled node - list concatenation not supported", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (a:A) RETURN a.list2 + a.list1 AS foo",
    );
    expect(results).toHaveLength(1);
    expect(results[0]).toEqual([4, 5, 1, 2, 3]);
  });
  ...
  test.fails(
    "[8] Returning label predicate expression - label predicates in RETURN not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[10] Return count aggregation over an empty graph - unlabeled nodes not supported",
    () => {
      const graph = createTckGraph();
  ...
  test.fails(
    "[custom] Return count aggregation over an empty graph with labeled nodes - comparison in RETURN not supported",
    () => {
      const graph = createTckGraph();
      const results = executeTckQuery(
        graph,
        "MATCH (a:A) RETURN count(a) > 0 AS result",
      );
      expect(results).toHaveLength(1);
      expect(results[0]).toBe(false);
    },
  ...
  test("[12] Projecting a list of nodes and relationships - list expressions in RETURN not supported", () => {
    ...
    const results = executeTckQuery(
      graph,
      "MATCH (n:A)-[r:T]->(m:B) RETURN [n, r, m] AS list",
    );
    expect(results).toHaveLength(1);
  });
  ...
  test("[13] Projecting a map of nodes and relationships", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (:A {name: 'a'})-[:R {since: 2020}]->(:B {name: 'b'})",
    );
    ...
    expect(result.node1.properties?.name || result.node1.get?.("name")).toBe(
      "a",
    );
    expect(result.node2.properties?.name || result.node2.get?.("name")).toBe(
      "b",
    );
  });
  ...
  test.fails(
    "[15] Fail when returning properties of deleted nodes - DELETE not fully supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE ({num: 1})");
      expect(() =>
        executeTckQuery(graph, "MATCH (n) DELETE n RETURN n.num"),
      ).toThrow();
    },
  );

  test.fails(
    "[16] Fail when returning labels of deleted nodes - DELETE not fully supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (:A)");
      expect(() =>
        executeTckQuery(graph, "MATCH (n:A) DELETE n RETURN labels(n)"),
      ).toThrow();
    },
  );

  test.fails(
    "[17] Fail when returning properties of deleted relationships - DELETE not fully supported",
    () => {
      const graph = createTckGraph();
      executeTckQuery(graph, "CREATE (:A)-[:T {num: 1}]->(:B)");
      expect(() =>
        executeTckQuery(graph, "MATCH ()-[r]->() DELETE r RETURN r.num"),
      ).toThrow();
    },
  );

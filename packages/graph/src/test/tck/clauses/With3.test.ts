...
describe("With3 - Forward multiple expressions", () => {
  test.fails(
    "[1] Forwarding multiple node and relationship variables - relationship variable in MATCH after WITH creates Cartesian product",
    () => {
  ...
  test("[custom-1] WITH forwarding multiple variables to RETURN", () => {
    ...
    executeTckQuery(
      graph,
      "CREATE (:A {name: 'a'})-[:T {id: 1}]->(:B {name: 'b'})",
    );

    const results = executeTckQuery(
      graph,
      "MATCH (a:A)-[r:T]->(b:B) WITH a, r, b RETURN a, r, b",
    );

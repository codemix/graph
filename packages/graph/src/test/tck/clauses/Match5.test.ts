...
describe("Match5 - Match variable length patterns over given graphs scenarios", () => {
  ...
  test.fails("[1] Handling unbounded variable length match - var length bounds issue", () => {
    // Variable length pattern doesn't respect bounds - returns too many matches
    const graph = createTckGraph();
    createBinaryTree(graph);
    const results = executeTckQuery(graph, "MATCH (a:A) MATCH (a)-[:LIKES*]->(c) RETURN c.name");
    expect(results).toHaveLength(14); // All descendants of n0
  });

  test.fails("[2] Handling explicitly unbounded variable length match - *.. syntax issue", () => {
  ...
  test.fails("[3] Handling single bounded variable length match 1 - *0 means all paths", () => {
    // *0 should return just the start node, but returns all paths
    const graph = createTckGraph();
    createBinaryTree(graph);
    const results = executeTckQuery(graph, "MATCH (a:A) MATCH (a)-[:LIKES*0]->(c) RETURN c.name");
    expect(results).toHaveLength(1);
    expect(results[0]).toBe("n0");
  });

  test.fails("[4] Handling single bounded variable length match 2 - bounds not respected", () => {
    // *1 should return 1-hop paths only
    const graph = createTckGraph();
    createBinaryTree(graph);
    const results = executeTckQuery(graph, "MATCH (a:A) MATCH (a)-[:LIKES*1]->(c) RETURN c.name");
    expect(results).toHaveLength(2);
    expect(results.sort()).toEqual(["n00", "n01"]);
  });

  test.fails("[5] Handling single bounded variable length match 3 - bounds not respected", () => {
    // *2 should return 2-hop paths only
    const graph = createTckGraph();
    createBinaryTree(graph);
    const results = executeTckQuery(graph, "MATCH (a:A) MATCH (a)-[:LIKES*2]->(c) RETURN c.name");
    expect(results).toHaveLength(4);
    expect(results.sort()).toEqual(["n000", "n001", "n010", "n011"]);
  });

  test.fails("[6] Handling upper and lower bounded variable length match 1 - bounds not respected", () => {
    ...
    expect(results.sort()).toEqual(["n0", "n00", "n000", "n001", "n01", "n010", "n011"]);
  });

  test.fails("[7] Handling upper and lower bounded variable length match 2 - bounds not respected", () => {
    ...
    expect(results.sort()).toEqual(["n00", "n000", "n001", "n01", "n010", "n011"]);
  });

  test.fails("[8] Handling symmetrically bounded variable length match, bounds are zero - bounds not respected", () => {
  ...
  test.fails("[9] Handling symmetrically bounded variable length match, bounds are one - bounds not respected", () => {
  ...
  test.fails("[10] Handling symmetrically bounded variable length match, bounds are two - bounds not respected", () => {
  ...
  test.fails("[12] Handling upper and lower bounded variable length match, empty interval 2 - bounds not respected", () => {
  ...
  test.fails("[13] Handling upper bounded variable length match, empty interval - *..0 syntax issue", () => {
  ...
  test.fails("[14] Handling upper bounded variable length match 1 - bounds not respected", () => {
  ...
  test.fails("[15] Handling upper bounded variable length match 2 - bounds not respected", () => {
  ...
  test.fails("[16] Handling lower bounded variable length match 1 - bounds not respected", () => {
  ...
  test.fails("[17] Handling lower bounded variable length match 2 - bounds not respected", () => {
  ...
  test("[18] Handling lower bounded variable length match 3", () => {
    ...
    expect(names.filter((n) => n.startsWith("n00") || n.startsWith("n01")).length).toBe(12);
  });

  test.fails("[19] Handling a variable length relationship and a standard relationship in chain, zero length 1 - bounds not respected", () => {
  ...
  test.fails("[20] Handling a variable length relationship and a standard relationship in chain, zero length 2 - bounds not respected", () => {
    // *0 should match just the start
    const graph = createTckGraph();
    createBinaryTree(graph);
    const results = executeTckQuery(
  ...
  test.fails("[21] Handling a variable length relationship and a standard relationship in chain, single length 1 - bounds not respected", () => {
  ...
  test.fails("[22] Handling a variable length relationship and a standard relationship in chain, single length 2 - bounds not respected", () => {
    // *1 should match 1-hop paths
    const graph = createTckGraph();
    createBinaryTree(graph);
    const results = executeTckQuery(
  ...
  test.fails("[25] Handling a variable length relationship and a standard relationship in chain, longer 3", () => {
  ...
  test.fails("[26] Handling mixed relationship patterns and directions 1", () => {
  ...
  test.fails("[27] Handling mixed relationship patterns and directions 2", () => {
    // Requires graph modifications
    const graph = createTckGraph();
    createBinaryTree(graph);
    executeTckQuery(

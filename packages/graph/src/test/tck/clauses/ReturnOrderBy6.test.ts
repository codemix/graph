...
describe("ReturnOrderBy6 - Aggregation expressions in order by", () => {
  ...
  test.fails("[2] Handle returned aliases inside an order by item which contains an aggregation expression - arithmetic on aggregates not supported", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(
      graph,
  ...
  test.fails("[3] Handle returned property accesses inside an order by item which contains an aggregation expression - arithmetic on aggregates not supported", () => {
    const graph = createTckGraph();
    const results = executeTckQuery(
      graph,
  ...
  test.fails("[4] Fail if not returned variables are used inside an order by item which contains an aggregation expression - semantic validation not implemented", () => {
    const graph = createTckGraph();
    expect(() => {
      executeTckQuery(
        graph,
  ...
  test.fails("[custom-1] ORDER BY with simple count aggregation - implicit grouping not supported", () => {
    const graph = createTckGraph();
